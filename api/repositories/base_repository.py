...
class BaseRepository(Generic[T]):
    ...
    async def list(self, limit: int = 100, offset: int = 0) -> List[T]:
        try:
            ...
            logger.debug(f"Listed {len(records)} {self.model.__name__}(s) with limit={limit} and offset={offset}.")
            return records
        except SQLAlchemyError as e:
    ...
    async def filter(self, **kwargs) -> List[T]:
        try:
            ...
            records = result.scalars().all()
            logger.debug(f"Filtered {len(records)} {self.model.__name__}(s) with criteria {kwargs}.")
            return records
        except SQLAlchemyError as e:
    ...
    async def update(self, instance: T, **kwargs) -> T:
        try:
            ...
            logger.info(f"Updated {self.model.__name__} with {primary_key_column.name}: {primary_key_value}.")
            return instance
        except SQLAlchemyError as e:
