...
from app.users.models import UsersTags
...
def update_profile():
    ...
    tags = Tag.query.filter(or_(Tag.allow_email == True, Tag.allow_sms == True)).all()
    ...
    if profile_form.validate_on_submit() and profile_form.submit.data:
        ...
        for tag in tags:
            user_tag = UsersTags.query.filter_by(
        ...
        for tag in tags:
            user_tag = UsersTags.query.filter_by(user_id=user.id, tag_id=tag.id).first()
            if user_tag:
                if tag.allow_email:
